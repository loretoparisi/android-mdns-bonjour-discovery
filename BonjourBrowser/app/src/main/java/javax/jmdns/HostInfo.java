...
class HostInfo
{
    private static Logger logger = Logger.getLogger(HostInfo.class.getCanonicalName());
    ...
    DNSRecord.Address getDNS4AddressRecord()
    {
        if (address != null) {
            if ((address instanceof Inet4Address) ||
               ((address instanceof Inet6Address) && (((Inet6Address) getAddress()).isIPv4CompatibleAddress())))
            {
                return new DNSRecord.Address(getName(), DNSConstants.TYPE_A, DNSConstants.CLASS_IN, DNSConstants.DNS_TTL, address);
            } else {
                logger.warning("Address " + address + " is not Inet4Address compatible!");
            }
        } else {
