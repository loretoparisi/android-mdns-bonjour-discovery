...
import org.jmdns.utils.DNSInfo;
...
public class JmDNS
{
    private static Logger logger = Logger.getLogger(JmDNS.class.getCanonicalName());
    /**
    ...
    private Thread unicastListener = null;
    ...
    public JmDNS() throws IOException
    {
        ...
        try
        {
            ...
            InetAddress addr = InetInfo.getIPAddress(hostname, 80);
            ...
            if (addr.isLoopbackAddress()) {
                logger.warning("Initializing with null, disallow loopback address " + addr);
                addr = null;
            } else {
                logger.fine("Initializing with address " + addr);
            }
        ...
        {
            final String fallbackName = "computer";
            logger.warning("Initializing JmDNS with null address an name of '" + fallbackName+"'");
            init(null, fallbackName);
        }
    ...
    {        
        unicastServerAddress = DNSInfo.getDNSServer();
        ...
        broadcastAddress = new HostInfo(address, name);
        ...
        try {
            ...
            logger.log(Level.WARNING, "unable to register shutdown hook", e);
            shutdown = null;
        }

        // Bind to multicast socket
        openMulticastSocket(broadcastAddress);
        multicastListener = new Thread(new SocketListener(multicastSocket), "JmDNS.MulticastListener");
        
        
        if (unicastEnabled()) {
            logger.info("Creating unicast socket");
            unicastSocket = new DatagramSocket();
            unicastListener = new Thread(new SocketListener(unicastSocket), "JmDNS.UnicastListener");
        } else {
            logger.info("Not creating unicast socket");
        }
        
        start(services.values());
    }
    
    private boolean unicastEnabled() {
        return !(unicastServerAddress == null || unicastServerAddress.equals(""));
    }
    ...
    private void openMulticastSocket(HostInfo hostInfo) throws IOException
    {
        ...
        multicastSocket = new MulticastSocket(DNSConstants.MDNS_PORT);

        if ((hostInfo != null) && (broadcastAddress.getInterface() != null))
        {
            multicastSocket.setNetworkInterface(hostInfo.getInterface());
        }
    ...
    {
        logger.fine("closeUnicastSocket()");
        if (unicastSocket != null)
        {
            ...
            try
            {
                ...
                {
                    //unicastSocket.interrupt();
                    unicastListener.join();
                }
            }
            catch (Exception exception)
            {
                logger.log(Level.WARNING, "closeUnicastSocket() Close socket exception ", exception);
            }
            unicastSocket = null;
        }
    }
    ...
    {
        try {
            return InetInfo.getHostname();
        } catch (IOException e) {
            logger.warning(e.toString());
        }
        return broadcastAddress.getName();
    }
    ...
    {
        return broadcastAddress.getAddress();
    }
    ...
    {
        logger.info("requestServiceInfo(): " + type + "," + name + ", " + timeout);
    ...
    {
        logger.info("Resolved service: " + info);
        ...
        if (list != null)
        {
            ...
            for (Iterator iterator = listCopy.iterator(); iterator.hasNext();)
            {
                ...
                logger.info("Informing: " + listener);
    ...
    public void removeServiceListener(String type, ServiceListener listener)
    {
        ...
        if (list != null)
        {
            logger.fine("removeServiceListener: Entering synchronized block" + new Exception().toString());
            synchronized (this)
            {
    ...
    public void registerService(ServiceInfo info) throws IOException
    {
        ...
        info.server = broadcastAddress.getName();
        info.addr = broadcastAddress.getAddress();
            ...
            synchronized (cache) {
                for (DNSCache.CacheNode j = cache.find(info.getQualifiedName().toLowerCase()); j != null; j = j.next())
                {
                    ...
                    if ((a.type == DNSConstants.TYPE_SRV) && !a.isExpired(now))
                    {
                        ...
                        if (s.port != info.port || !s.server.equals(broadcastAddress.getName()))
                        {
                            logger.finer("makeServiceNameUnique() JmDNS.makeServiceNameUnique srv collision:" + a + " s.server=" + s.server + " " + broadcastAddress.getName() + " equals:" + (s.server.equals(broadcastAddress.getName())));
    ...
    private void handleResponse(DNSIncoming msg) throws IOException
    {
        ...
        for (Iterator i = msg.answers.iterator(); i.hasNext();)
        {
            ...
            //System.out.println("examining: " + rec.name);
            ...
            if (c != null)
            {
                ...
                {
                    //System.out.println("record not expired");
                    c.resetTTL(rec);
                    rec = c;
                }
            }
            else
            {
                ...
                {
                    isInformative = true;
                    logger.info("Adding " + rec + " to cache");
                    cache.add(rec);
                } else {
                    //System.out.println("record expired, not adding to cache");
                }
            }
            switch (rec.type)
            {
            ...
            if (isInformative)
            {
                //System.out.println("informative: " + rec.name);
                updateRecord(now, rec);
            } else {
                //System.out.println("not informative: " + rec.name);
            }
        }
    ...
    private void send(DNSOutgoing out) throws IOException
    {
        ...
        if (!out.isEmpty())
        {
            ...
            if (unicastEnabled()) {
                if (unicastSocket != null) {
                    packet.setAddress(InetAddress.getByName(unicastServerAddress));
                    packet.setPort(DNSConstants.DNS_PORT);
                    unicastSocket.send(packet);
                } else {
                    logger.log(Level.WARNING, "Unicast enabled but unicastSocket is null");
                }
            } 
        }
    }
    ...
    class SocketListener implements Runnable
    {
        ...
        public SocketListener(DatagramSocket dgramSocket) {
            ...
            try
            {
                ...
                while (state != DNSState.CANCELED)
                {
                    ...
                    try
                    {
                        ...
                        if (broadcastAddress.shouldIgnorePacket(packet))
                        {
                            logger.fine("Ignoring packet " + packet);
                            continue;
                        }
                        ...
                        synchronized (ioLock)
                        {
                            if (msg.isQuery())
                            {
                                ...
                                // this is not an 'else' because I think we want to
                                // broadcast responses to everyone, even for odd queries.
                            ...
                            else
                            {
                                /*if (packet.getPort() != DNSConstants.MDNS_PORT)
                                {
                                    System.out.println("*** Handling message from port " + packet.getPort());
                                }*/
                                handleResponse(msg);
                                /*if (packet.getPort() != DNSConstants.MDNS_PORT)
                                {
                                    System.out.println("*** Done handling message from port " + packet.getPort());
                                }*/
                            }
                        }
                    }
            ...
            synchronized (ioLock)
            {
                ...
                try
                {
                    ...
                    if (state == taskState && task == this)
                    {
                        ...
                        out.addQuestion(new DNSQuestion(broadcastAddress.getName(), DNSConstants.TYPE_ANY, DNSConstants.CLASS_IN));
                        DNSRecord answer = broadcastAddress.getDNS4AddressRecord();
                        if (answer != null)
                        {
                            out.addAuthorativeAnswer(answer);
                        }
                        answer = broadcastAddress.getDNS6AddressRecord();
                        if (answer != null)
                        {
                            out.addAuthorativeAnswer(answer);
                        }
                    ...
                    for (Iterator i = list.iterator(); i.hasNext();)
                    {
                        ...
                        synchronized (info)
                        {
                            if (info.getState() == taskState && info.task == this)
                            {
                                ...
                                out.addAuthorativeAnswer(new DNSRecord.Service(info.getQualifiedName(), DNSConstants.TYPE_SRV, DNSConstants.CLASS_IN, DNSConstants.DNS_TTL, info.priority, info.weight, info.port, broadcastAddress.getName()));
                            }
                        }
                    }
                    if (out != null)
                ...
                for (Iterator i = list.iterator(); i.hasNext();)
                {
                    ...
                    synchronized (info)
                    {
                        if (info.getState() == taskState && info.task == this)
                        {
                            ...
                            out.addAnswer(new DNSRecord.Service(info.getQualifiedName(), DNSConstants.TYPE_SRV, DNSConstants.CLASS_IN, DNSConstants.DNS_TTL, info.priority, info.weight, info.port, broadcastAddress.getName()), 0);
            ...
            for (Iterator i = in.questions.iterator(); i.hasNext();)
            {
                ...
                if (entry instanceof DNSQuestion)
                {
                    ...
                    iAmTheOnlyOne &= (q.type == DNSConstants.TYPE_SRV
                        ...
                        || broadcastAddress.getName().equalsIgnoreCase(q.name)
                        ...
                        for (Iterator iterator = in.questions.iterator(); iterator.hasNext();)
                        {
                            ...
                            if (entry instanceof DNSQuestion)
                            {
                                ...
                                { // I ama not sure of why there is a special case here [PJYF Oct 15 2004]
                                    if (broadcastAddress.getName().equalsIgnoreCase(q.getName()))
                                    {
                                ...
                                logger.fine("Got request for record of type " + type);
                                ...
                                switch (type)
                                {
                                    case DNSConstants.TYPE_A:
                                        {
                                            ...
                                            DNSRecord answer = broadcastAddress.getDNS4AddressRecord();
                                            logger.fine("Answering request " + q + " for A record with " + answer);
                                            if (answer != null)
                                            {
                                                answers.add(answer);
                                            }
                                    ...
                                    case DNSConstants.TYPE_AAAA:
                                        {
                                            // Answer a query for a domain name
                                            DNSRecord answer = broadcastAddress.getDNS6AddressRecord();
                                            logger.fine("Answering request " + q + " for AAAA record with " + answer);
                                            if (answer != null)
                                            {
                                                answers.add(answer);
                                            }
                                    ...
                                    case DNSConstants.TYPE_PTR:
                                        {
                                            ...
                                            logger.fine("Answering request for PTR record");
                                            ...
                                            for (Iterator serviceIterator = services.values().iterator(); serviceIterator.hasNext();)
                                            {
                                                ...
                                                if (info.getState() == DNSState.ANNOUNCED)
                                                {
                                                    if (q.name.equalsIgnoreCase(info.type))
                                                    {
                                                        ...
                                                        answers.add(new DNSRecord.Service(info.getQualifiedName(), DNSConstants.TYPE_SRV, DNSConstants.CLASS_IN | DNSConstants.CLASS_UNIQUE, DNSConstants.DNS_TTL, info.priority, info.weight, info.port, broadcastAddress.getName()));
                ...
                if (state == DNSState.ANNOUNCED)
                {
                    if (++count < 3)
                    {
                        try {
                            ...
                            logger.log(Level.WARNING, "too many answers for one packet!");
                        }
                    }
                    else
                    {
            ...
            try
            {
                if (++count < 3)
                {
                    ...
                    for (int i = 0; i < infos.length; i++)
                    {
                        ...
                        out.addAnswer(new DNSRecord.Service(info.getQualifiedName(), DNSConstants.TYPE_SRV, DNSConstants.CLASS_IN, ttl, info.priority, info.weight, info.port, broadcastAddress.getName()), 0);
    ...
    public void close()
    {
        if (state != DNSState.CANCELED)
        {
            ...
            logger.fine("unregisterAllServices()");
            unregisterAllServices();
            
            logger.fine("disposeServiceCollectors()");
            disposeServiceCollectors();
